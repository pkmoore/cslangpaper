...
Linux supports several special file types and
writing to and
reading from these files requires special procedures.
The SEA technique
may simulate the presence of such a file by
modifying
the return value
of a {\tt stat} call.
PORT improves on the process described by Moore et al. 
for employing such an anomaly
by simplifying the
process of describing a target pattern
