...
Writing to and
reading from these files types requires special procedures.
The SEA technique
may simulate the presence of such a file by
modifying
the return value
of a {\tt stat} call.
While Moore et al. had to manually
write a script
for each anomaly
to process system call traces, 
PORT improves on this process by simplifying the description and directing where and how to make the required modifications.
...
are not well suited to this task. To leverage
SEA's success beyond system calls to encompass other activity types,
such as calls to library functions
and remote procedure calls, we needed to develop a novel tool.
Such a tool would be  agnostic to the way an application's activity has been recorded, and as such would require its own  distinct language. 
...
A simple model,
such as a deterministic finite automaton (i.e. a DFA or a FSA),  written enhanced to operate on complex
structures would appear to be sufficient.
Unfortunately, due to  dependencies
between activities in these patterns
across time, the simple model will not work.
...
One possibility is to deploy more expressive automata models, such as register automata~\cite{DBLP:journals/tcs/KaminskiF94} or session automata~\cite{DBLP:journals/corr/BolligHLM14}. Yet, these models cannot produce a modified output.  As the 
