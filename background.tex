...
insight.
First was that, with the correct tool,
SEA's success in identifying environmental bugs
could be expanded 
beyond strace and system calls
to encompass other activity types,
such as calls to library functions
and remote procedure calls.
This could be achieved by designing such a tool
can operate in an agnostic fashion with respect to the way an application's activity has been recorded.
in a format agnostic fashion.
...
they do not permit  the production of a modified output events  processing.
We learned from the  SEA researchers that the ability to
{\textit modify} activity allows researchers to create scenarios for the application that ensure
failure rather than  waiting for it to possibly  perform
problematic sequences.
These capabilities are provided by several feature-rich event processing
languages and libraries.
Unfortunately, we found that, while some of these languages can modify and output
incoming events,
...
(which can be executed using the code and tools available at: \textit{Link Removed for Blinding Purposes})
