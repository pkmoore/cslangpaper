...
\begin{figure}
  \begin{lstlisting}[gobble=4,xleftmargin=.8em]
    event open {filename: String@0, filedesc: Number@ret};
...
we consider a program that recognizes a system call sequence wherein an
...
a file.  The final three statements describe the order in which the
{\tt open()}, {\tt read()}, and {\tt close()} are expected.
The {\tt read()} event is preceded by NOT indicating that a sequence
should be rejected if this event is encountered at any point between the
{\tt open()} and {\tt close()} events.
