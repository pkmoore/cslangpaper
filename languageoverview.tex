...
\begin{figure}
  ...
  \caption{A CSlang transducer individually processes each item in an input
  sequence.  For each item in the sequence, the transducer updates it's
  internal state and produces output according to the rules described in
  its program.}
...
\preston{If these are events then what are we calling structure
definitions?}
