...
The language allows the user to define a mapping of concrete events to abstract events by hiding parameter values that are irrelevant for the particular processing task at hand, and by grouping events that are semantically related but have different event identifiers together into a single abstract event. The actual description of the transducer then refers to these abstract events.

The transducer itself is described by a finite sequence of \emph{actions}. A state of the transducer keeps track of the next action to be executed, as well as a valuation of a finite number of \emph{registers} that hold data values. The registers of the transducers are implicitly defined by the actions. 
...
\mathsf{body} ::= {} & \mathsf{action}^*\\
\mathsf{action} ::= {} & \mathsf{assignment} \mid \mathsf{event}\\
\mathsf{assignment} ::= {} & \mathit{regid} \texttt{ <- } e\\
\mathsf{event} ::= {} & \mathit{id}\,(p) \;\mathtt{with}\; b \texttt{ -> } \mathit{id}\,(e)\\
