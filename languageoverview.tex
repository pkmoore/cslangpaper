...
The execution of an action involves reading the next event from the input stream, which may update some of the data registers, and writing the next event in the output stream.
...
\begin{lstlisting}[numbers=none,xleftmargin=0em,gobble=2,columns=strict]
  event bothread {read filedesc: Number@0}
                    | {recv filedesc: Number@0};
\end{lstlisting}
