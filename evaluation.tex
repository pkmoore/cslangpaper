...
\end{lstlisting}
\end{tabular}
\caption{The above is a grouping of Windows API calls for opening or creating a Windows
...
Windows makes to actually perform the open or create operation.  This
operation grouping and execution semantics can be expressed
...
detected, would indicate that a piece of software was
installing a Windows registry key that would cause some malicious action
...
SET, and CLOSE operations and searches for a pattern that an application is
...
is focused on identifying specific
...
recorded using {\tt strace} while processing a request.
The strace
...
\caption{This CSlang program matches patterns where a server application
  accepts a connection, receives a request, sends a response, and closes a
...
CSlang's
processing time increases in line with the total number system calls
...
optimized implementation could see performance gains in this area.
Further,
