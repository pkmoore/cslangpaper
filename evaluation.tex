...
if it would be effective in real world situations.
...
We selected remote procedure calls as a logical next step and chose the JSONRPC and XMLRPC APIs for this purpose. Both  are widely used and have well
...
and ``update'' calls. If the pattern is found, the final update call's
...
a Windows registry key capable of causing some malicious action  has been installed.
