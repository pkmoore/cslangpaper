...
usbhid({src: ?src, dst: ?dst, data: ?num1}) -> usbhid({data: ->num2});
  \end{lstlisting}
  \caption{A demonstration port program that matches USB activity that indicates the '1' key is being pressed and transforms it to a new frame where the '2' key is being pressed}
  \label{fig:USB}
\end{figure}
...
we implemented support for streams of USB activity. This format was chosen because of its ubiquity and reliance on numerous parties correctly implementing a standard protocol.
...
As a test scenario, we settled upon the recent type of usb-based attack known as BADUSB~\cite{badusb}.
...
The above successes show that PORT is easily capable of extension to activity representations beyond system calls.
...
of programs that were curated rather than randomly selected.  This was done due to the selected program's popularity and the lack of time to complete an exhaustive exploration of all candidates. There may be programs that would diverge from the performance trend we report above. Future work can determine how widely this phenomena occurs and if any subsequent modifications are required.
